...
class PhysicalModel:
    ...
    gamma: float = 1e-3  # [F]/[v] = [kg/sec] linear drag air resistence (dominant in low speeds)
    c_drag: float = 1e-4  # [F]/[v^2] quadratic drag air resistence (dominant in high speeds)
    ...
    energy_conversion_factors = {
        'activity_efficiency': 0.25,  #0.25,  # propulsion force to energy factor (higher -> efficient)
        ...
        'brain_consumption': 0.01,  # [E]/[no units] convert brain size to brain consumption energy
        'height': 20,  # [E]/[h]  # convert height to height energy
        'mass': 20  # [E]/[m] = [v^2] = [m^2/sec^4]  convert mass to mass energy
    }
    ...
    trait_energy_params_dict = {
        'height': {'factor': 0.025, 'rate': 5e-4},  # [factor] = [1], [rate] = [1/sec]
        'mass': {'factor': 0.025, 'rate': 2e-4}  # [factor] = [1], [rate] = [1/sec]
    }
